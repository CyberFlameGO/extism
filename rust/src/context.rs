...
impl Context {
    ...
    pub fn active_plugin_memory(&mut self, mem: MemoryBlock) -> Option<*mut u8> {
        match self.lock().active_plugin() {
            Some(p) => unsafe { Some(p.memory.data_mut().as_mut_ptr().add(mem.offset)) },
            None => None,
        }
    }

    pub fn active_plugin_alloc(&mut self, n: usize) -> Result<MemoryBlock, Error> {
        match self.lock().active_plugin() {
    ...
    pub fn active_plugin_free(&mut self, block: MemoryBlock) {
        match self.lock().active_plugin() {
    ...
    pub fn active_plugin_memory_block(&self, offs: usize) -> Option<MemoryBlock> {
        self.lock().active_plugin().and_then(|plugin| {
            plugin
                .memory
                .block_length(offs)
